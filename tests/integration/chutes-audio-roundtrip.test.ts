...
describe('Audio TTS-to-STT Round-Trip (Integration)', () => {
	...
	describe('Speech-to-Text', () => {
		testOrSkip('should transcribe audio buffer', async () => {
			...
			const ttsResponse = await fetch(`${TTS_CHUTE_URL}/speak`, {
				method: 'POST',
				headers: {
					'Authorization': `Bearer ${API_KEY}`,
					'Content-Type': 'application/json',
			...
			const sttResponse = await fetch(`${STT_CHUTE_URL}/transcribe`, {
				method: 'POST',
				headers: {
					'Authorization': `Bearer ${API_KEY}`,
					'Content-Type': 'application/json',
			...
			const chunks = await sttResponse.json() as Array<{ start: number; end: number; text: string }>;
